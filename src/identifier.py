...
class Identifier:
    ...
    async def compute_known_embeddings(self):
            ...
            for file in os.listdir(label_path):
                if (
                    ...
                    for face, _, _ in faces:
                        embed = await self.encoder.encode(face, is_ir)
    ...
    async def get_detections(self, img):
        ...
        for face, face_region, _ in faces:
            match, conf = await self.compare_face_to_known_faces(face, is_ir)
            detection = {
                "confidence": conf,
                "class_name": match,
                "x_min": face_region["x"],
    ...
    async def compare_face_to_known_faces(
        self, face, is_ir, unknown_label: str = "unknown"
    ):
        """
        ...
        """
        source_embed = await self.encoder.encode(face, is_ir)
        match = None
        min_dist = math.inf
